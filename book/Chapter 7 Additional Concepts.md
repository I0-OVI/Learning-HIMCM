...
🚧
In order to address this, I would introduce **embedding**. In the previous part, we only use one numerical values to describe the categories but this time we assign multiple values to them. In particularly, we input with vectors instead of single numbers. Imagine we use n numbers to describe the current categories. We will get the n dimensional vectors. By projecting these vectors to n dimensional space, we will know the difference between each categories. This vector is calculated during the training. 
