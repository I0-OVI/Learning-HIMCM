...
#### Quantization
There are many non-numerical features in the dataset such as names, location, jobs and so on. Since they have complicated meaning for each context which is hard to process by many traditional machine learning models, we have to convert them into numerical values using certain rules. For example, consider a feature describes the location within one city, we could divide the current city into several regions like *inner ring*, *central ring* and *outer ring* and assign numerical numbers to them. (e.g., 1,2 and 3) This method is not limited for the non-numerical data. It could be applied to numerical features when the value range is very large. Suppose we have an income table of 100 individuals with some extreme values, which may negatively affect the performance of model. In order to address this, we could discretize income to levels: 0-3000 as low, 3000-20000 as medium and above 20000 as high. Each level is assigned a numerical value.

However, there is a potential problem when using quantization. Some of the features are inherently non-comparable, yet after quantization they may appear to be ordered or numerically related. This may produce a misleading bias for the model, since assigning numerical values emphasize there are some linear relationship between these values. Suppose we assign numerical values to jobs, such as doctor = 1, driver = 2 and police = 3. So here is the question: Does this job comparable? Absolutely not, no meaningful ordering exists between them. Quantization may also distort continuous data when certain threshold is applied. Suppose we have two income values: 19999 and 20001. In the previous criteria, the first one is assigned by 2 and the next one is assigned by 3, meaning the second one is 1.5 times better than the first one. But actually they only have tiny difference between their incomes and the model exaggerates the difference between them.

In order to address this, I would introduce **embedding**. Instead of representing each category with a single numerical value, embedding represents each category as a learnable vector. Specifically, each category is mapped into a n-dimensional vector, where n is the embedding dimension. These vectors are not manually assigned or calculated. They are learned during model training through back-propagation (one part in the neural network). Embedding allows the model to capture the similarities between each category. It is important to clarify that **embedding does not necessarily mean dimension expansion**. Embedding focuses on how a feature is represented, while dimension expansion focuses on using more features to describe the entire case. In other words, embedding describes a feature using multiple aspects, whereas dimension expansion introduces additional independent features. 
